...
<main class="app-content">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="tile">
                <div class="tile-body">
                    <form class="form-horizontal" action="/edit_order/${orderResponse.order_id}" method="post">
                        <div class="form-group row">
                            <label class="control-label col-3">Pin code:</label>
                            <div class="col-3">
                                <input class="form-control" type="text" maxlength="7" placeholder="" name="pin" value="${orderResponse.pin}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-3 ">Device:</label>
                            <div class="col-3">
                                <input class="form-control"  id="device" value="${orderResponse.device}"/>
                                <input class="form-control" type="hidden" id="device_Id" name="device_Id" value="${orderResponse.deviceId}"/>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-3 ">
                                <select id="multiselect" multiple="multiple" name="problems">
                                    <c:forEach var="items" items="${list}">
                                        <c:set var="a" value=""  />
                                        <c:forEach var="pro" items="${orderResponse.problems}">
                                            <%--<option value="<c:if test="${pro.id == items.id}"/>" selected>${items.name}</option>--%>
                                            <c:if test="${pro.id == items.id}"> <c:set var="a" value="selected"  /></c:if>

                                        </c:forEach>
                                        <option value="${items.id}" ${a}>${items.name}</option>
                            ...
                            <br>
                            <div class="col-3">
                                <input class="form-control" type="text" placeholder="descriptions" name="info" value="${orderResponse.info}">
                            </div>
